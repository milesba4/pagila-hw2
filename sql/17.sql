...
SELECT rev AS rank, title, revenue, SUM(revenue) OVER (ORDER BY revenue DESC) AS "total revenue" FROM (SELECT rank() OVER (ORDER BY revenue DESC) AS rev, title, revenue FROM (SELECT title, COALESCE(SUM(amount), 0.00) AS revenue FROM film LEFT JOIN inventory USING (film_id) LEFT JOIN rental USING (inventory_id) LEFT JOIN payment USING(rental_id) GROUP BY title ORDER BY revenue DESC) AS t ORDER BY revenue DESC) AS profit ORDER BY rank, title;
